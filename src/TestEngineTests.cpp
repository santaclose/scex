...
void ste::ImGuiController::ConsoleLog(const std::string& log)
{
    consoleLogs.push_back(log);
}
void ste::ImGuiController::ShowConsole()
{
    ImGui::Begin("Console");
    for (const std::string& x : consoleLogs)
        ImGui::Selectable(x.c_str());
    ImGui::End();
}

void ste::ImGuiController::RegisterTestEngineTests(ImGuiTestEngine* e)
{
    ImGuiTest* t = NULL;
